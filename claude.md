...
- **コマンド例を勝手に書かない** — 「何を確認するか」だけ伝える。具体的なコマンドは早野さんに聞かれたときだけ、実際の環境情報（このファイルに記載のパス・ホスト名・ブランチ名）を使って書く
- **プレースホルダー禁止** — `/path/to/project`、`your-server`、`example.com` のような仮の値を絶対に使わない。情報が足りなければ聞く
- **ターミナル操作は早野さんが行う** — Claude がコマンドを実行しない。求められたら指示だけ出す
- **ファイル編集の前にかならず現状確認** — 対象ファイルの中身を見てから修正方針を提案する
